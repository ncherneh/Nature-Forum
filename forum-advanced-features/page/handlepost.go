...
func VisualPost(w http.ResponseWriter, r *http.Request) {
	...
	canChange, err := strconv.ParseBool(r.FormValue("CanChange"))

	if err != nil {
		canChange = false
	}

	canChangeComment, err := strconv.ParseBool(r.FormValue("CanChangeComment"))
	...
	if isLoggedIn {
		p_c.NewNotification = CountOfNewNotification(users.ID) > 0
	}
...
func EditPost(w http.ResponseWriter, r *http.Request) {
	...
	post_title := r.FormValue("title-edit-post")
	post_content := r.FormValue("text-edit-content")
	...
	db.EditPostData(postID, post_title, post_content)
