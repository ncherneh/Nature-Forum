...
const MaxUploadSize = 20000000 // 20 MB
...
func CreatePost(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		//image
		file, handler, err := r.FormFile("image-create-post")
		...
		if err != nil {
			...
			if handler.Size > MaxUploadSize {
				data := struct {
					...
					Err:        1,
					Message:    "Uploaded file is too large, please upload a file less than 20MB",
					Categories: categories,
					UserID:     users,
					IsLoggedIn: isLoggedIn,
				}
			...
			if err != nil {
				...
				HandleError(w, http.StatusInternalServerError, "Failed to save the image: "+err.Error())
				return
			}
			defer f.Close()
			io.Copy(f, file)
...
func DeletePost(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Failed to delete the image file", http.StatusInternalServerError)
		return
	}
